...
def test_create_evidence_records_initial_custody_event():
    ...
    def get_record(model, record_id):
        if model is __import__("models.case", fromlist=["Case"]).Case:
            return case
    ...
    with patch("evidence_management.db.session.get", side_effect=get_record), patch(
        "evidence_management._require_case_access"
    ), patch.object(db.session, "add", side_effect=added.append), patch.object(
        db.session, "flush", side_effect=lambda: setattr(added[0], "id", 21)
