...
class Evidence(db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        # Transient instances have no committed SQLAlchemy history. Preserve
        # creation-time values so the immutability guard can also be invoked
        # directly before the object is persisted.
        self._initial_evidence_id = self.evidence_id
        self._initial_sha256_hash = self.sha256_hash
...
def _prevent_evidence_identity_change(mapper, connection, target):
    ...
    if history.transient:
        if (
            hasattr(target, "_initial_evidence_id")
            and target.evidence_id != target._initial_evidence_id
        ):
            raise ValueError("Evidence identifiers are immutable")

        if (
            hasattr(target, "_initial_sha256_hash")
            and target.sha256_hash != target._initial_sha256_hash
        ):
